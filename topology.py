...
portal.context.defineParameter("mem", "Memory to be used in gigabytes", portal.ParameterType.INTEGER, 40)
...
def raw_machine(lan, node_name, node_id, node_ip, params, node_offset):
    ...
    node.addService(pg.Install(url="https://github.com/hensg/emulab-parallel-checkpoint/releases/download/0.0.1/bft-smart.tar.gz", path="/srv"))
    node.addService(pg.Install(url="https://github.com/hensg/emulab-parallel-checkpoint/releases/download/0.0.1/emulab.tar.gz", path="/srv"))
    ...
    node.addService(pg.Execute(
        shell='bash',
        command='sudo /srv/emulab-parallel-checkpoint/install_service.sh {username} {id} {interval} {threads} {initial_entries} {checkpoint_interval} {parallel} {num_disks} {mem}'.format(
            username=params.u,
            id=node_id,
            interval=params.ri,
            threads=params.th,
